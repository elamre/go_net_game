...
import (
	...
	packets2 "github.com/elamre/go_net_game/net_systems/lobby_system/lobby_system_packets"
	"log"
	"strings"
	"time"
)
...
type LobbyServerSystem struct {
	...
	playersInLobby map[int32]*net.ServerPlayer
	playersInRoom  map[int32]*common.NetRoom
...
func (s *LobbyServerSystem) ClientDisconnect(c *net.ServerPlayer) {
...
func (s *LobbyServerSystem) roomPacketCallback(c *net.ServerPlayer, d common_system2.ServerRegulator, pack packet_interface.Packet) {
	...
	if t.Action == packets2.RoomCreateAction {
		...
		newRoom := common.NetRoom{
			...
			Players:  []*net.NetPlayer{&c.NetPlayer},
		}
