...
import (
	"encoding/json"
	"github.com/elamre/go_net_game/net/webrtc/internal/webrtc_shared"
	"io"
	"log"
	"net"
	"net/http"
...
func (s *Server) handleSDP(w http.ResponseWriter, r *http.Request) {
	...
	for _, conn := range s.connections {
		...
		if s.OnConnection != nil {
			log.Printf("Server on connect: %p, %+v", foundConn, foundConn)
...
func (conn *Connection) onDataChannelClose() {
	...
	if conn.onDisconnect != nil {
		log.Printf("Server on disconnect: %+p, %+v", conn, conn)
